...
function isGeoPresent() {
    return geoLayer
        ? map.hasLayer(geoLayer) &&
        leafletPip.pointInLayer(marker.getLatLng(), geoLayer).length > 0
        : false;
}
//function for adding marker
function addmarker(coord) {
    mape.addEventListener("keydown", markerMove);
    if (coord[0] < -180) {
        coord[0] = 180;
    ...
    if (marker) {
        ...
        findplacename(marker).then((place) => {
            updateLiveRegion(
                `Marker is on ${place}. Use arrow keys to navigate`,
                true
            );
        });
...
function addAdPointer(coord) {
    mape.removeEventListener("keydown", markerMove);
    if (marker) {
        marker.remove();
        poly && poly.remove();
        poly = null;
...
const throttledFunction = _.throttle((event) => {
    ...
    if (event.code == "KeyZ") {
        ...
        try {
            ...
            let zoom =
                scale < 1000
                    ? parseInt(scale) + " meters"
                    : parseInt(scale / 1000) + " Kilo meters";
            console.log(zoom);
...
const markerMove = _.throttle((event) => {
    if (event.code == "KeyD") {
        ...
        d +=
            "," + eastDistance < 1000
                ? parseInt(eastDistance) + " meters"
                : parseInt(eastDistance / 1000) + " Kilo meters";
        d += " to east. ";
        d +=
            "," + southDistance < 1000
                ? parseInt(southDistance) + " meters"
                : parseInt(southDistance / 1000) + " Kilo meters";
        d += " to south. ";
    ...
    if (event.code == "KeyF" || event.code == "Enter") {
        //for stating place name and select place
        event.preventDefault();
    ...
    if (
        ...
        //marker movement on map using arrowkeys
        if (!event.shiftKey) {
            perkeydist =
                ((40075016 * Math.cos((marker.getLatLng().lat * Math.PI) / 180)) /
                    Math.pow(2, map.getZoom() + 8)) *
                ...
                case "ArrowUp":
                    if (isGeoPresent()) {
                        if (northDistance <= perkeydist) {
                            addmarker([
                                borderpoints.north[0] - 0.000001,
                                borderpoints.north[1],
                            ]);
                            bordertouched("north");
                        } else {
                ...
                case "ArrowDown":
                    if (isGeoPresent()) {
                        if (southDistance <= perkeydist) {
                            addmarker([
                                borderpoints.south[0] + 0.000001,
                                borderpoints.south[1],
                            ]);
                            bordertouched("south");
                        } else {
...
const fetchElevation = _.throttle(async (point) => {
    fetch(
        `https://api.open-elevation.com/api/v1/lookup?locations=${marker.getLatLng().lat
        ...
        .then((data) => {
            if (data.results[0].elevation) {
                document.getElementById(
                    "elevation"
                ).innerHTML = `Altitude: ${data.results[0].elevation} meters`;
            }
        })
        .catch((error) => {
            console.error("Error in fetching place name:", error);
