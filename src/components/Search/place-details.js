...
export async function fetchDetails(result) { //fetch details of the place based on country, state, district, river, or other
    ...
    if (adminLevel <= 2) return fetchCountryDetails(display_name, osm_type, osm_id);
    if ([3, 4].includes(adminLevel)) return fetchStateDetails(display_name, osm_type, osm_id);
    if (adminLevel === 5) return fetchDistrictDetails(display_name, osm_type, osm_id);
  ...
  async function fetchRiverDetails(tags, result) {
    ...
    return formatOutput(`River: ${tags.name || "Unknown"}`, details, riverTemplate);
  }
  ...
  async function fetchMultipleClaims(wikidataDetails, claimKey) {
    ...
    return Promise.all(claims.map(async (claim) => fetchEntityLabel(claim.mainsnak.datavalue?.value.id)))
